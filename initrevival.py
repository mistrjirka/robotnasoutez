...
class Robot:
	def __init__(self, commands = [{"direction":"right", "toDo": [-180, 180], "degreesDelay": 50}, {"direction":"left", "toDo": [180, -180], "degreesDelay": 50}, {"direction":"backward", "toDo": 500 * -1, "degreesDelay": 50}, {"direction":"forward", "toDo": 500, "degreesDelay": 50}], colorS = [{"val": [3,6], "toDo": "forward"}, {"val": [2], "toDo": "right"}, {"val": [5], "toDo": "left"}], motor1 = LM("outC"), motor2 = LM("outB")):
		...
		if whatToDo != self.doCache or motorAreRunning == False:
			for i in self.commands:
				if i["direction"] == whatToDo:
					...
					self.motors[0].run_forever(speed_sp=500)
					...
					if str(type(i["toDo"]).__name__) == "list":
						...
						self.motors[0].run_to_rel_pos(position_sp=i["toDo"][0], speed_sp=500, stop_action="hold")
						self.motors[1].run_to_rel_pos(position_sp=i["toDo"][1], speed_sp=500, stop_action="hold")
						motorAreRunning = True
						for j in i["toDo"]:
							if j > 0:
								sleep(j/300)
